...
⍝ === End of variables definition ===
    ...
    ∇ r←larg Call rarg;service;method;arg;ok;cmd;port;req;hdr;z;page;protocol;host;lchost;req2send;mask;tmp;ss;http;body;resp;soapaction;xmlns;cert;secure;ssl;length;t;m;certdir;chunked;chunk;buffer;chunklength;done;data;datalen;header;wr;getchunklen;h2d;len
      ...
      h2d←{⎕IO←0 ⋄ 16⊥'0123456789abcdef'⍳HTTPUtils.lc ⍵} ⍝ hex to decimal
...
⍝    :Until ('BlockLast'≡3⊃z)∨(length≤⍴resp)∨':Envelope>'{⍺≡(-⍴⍺)↑⍵}resp   ⍝ Until error or all data received
    ...
    ∇ R←HandleRequest(cmd session);method;arg;ok;type;res;status;hdr;mpvm;TIME;buffer;name;ns;api;wsdl;expfns;WSDL;ServiceName;ExportedFns;API;rc;svc;ServiceNS;host;lcprefix
      ...
      :If ~0∊⍴Prefix
          ...
          cmd.Input←lcprefix{0=i←1↑{⍵/⍳⍴⍵}⍺⍷HTTPUtils.lc ⍵:⍵ ⋄ (i↑⍵),(i+⍴⍺)↓⍵}cmd.Input
      :EndIf
    ...
    ∇ {r}←{svc}Run arg;thread;srvname;port
                    ...
                    ⍝ arg[5] - prefix to remove if running behind other web server via CGI
                    ⍝
      ...
      port thread srvname URL Prefix←5↑arg,(⍴,arg)↓8080 0 'HTTPSRV' 'localhost' ''
    ...
    ∇ {r}←{svc}RunSecure arg;thread;srvname;port;certpath;cert;sslflags
                    ...
                    ⍝ arg[5] - prefix to remove if running behind other web server via CGI
                    ⍝ arg[6] - path for certificates, if empty or non-existent, use path from #.Samples.CertPath
                    ⍝ arg[7] - server certificate (DRC.X509Cert instance) or empty/non-existent if not using certificate
                    ⍝ arg[8] - SSL flags (default 96)
                    ⍝
                    ⍝ If Service is supplied, the server runs only that Web Service with the supplied port and server name
                    ⍝ Note: Multiple Web Services can be run simultaneously provided that each one is run in a separate thread
                    ⍝       and has a separate port and server name
      ...
      {}DRC.Init''
      ⎕EX'STOP' ⍝ remove STOP flag
      port thread srvname URL Prefix certpath cert sslflags←8↑arg,(⍴,arg)↓445 0 'HTTPSRV' 'localhost' '' ''(⎕NEW DRC.X509Cert)96
      :If 0∊⍴srvname ⋄ srvname←'HTTPSRV' ⋄ :EndIf
      :If 0∊⍴URL ⋄ URL←'localhost' ⋄ :EndIf
      :If 0=⎕NC'svc' ⋄ Service←''
      :Else ⋄ Service←⍕svc
      :EndIf
      :If thread
          r←certpath WebServer.HttpsRun&'##.HandleRequest'port srvname cert sslflags
      :Else
          r←certpath WebServer.HttpsRun'##.HandleRequest'port srvname cert sslflags
      :EndIf
    ∇
