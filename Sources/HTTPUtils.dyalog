...
∇ HTTPResponse←ParseHTTPResponse resp;status;rest;tmp;version;reason;code;headers;body
                    ⍝ Parses an HTTP response
                    ⍝ resp is a character vector HTTP response
 status rest←NL split resp
 version tmp←' 'split status
 code reason←' 'split tmp
 headers body←(NL,NL)split rest
 headers←headers spliton NL
